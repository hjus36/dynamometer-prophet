...
- `FB_Rpm`, `FB_Torque`, `Temp**`, `Press**`는 값 범위상 **×10 스케일(소수점 1자리)** 형태일 가능성이 높다고 가정하여 `값 ÷ 10` 변환을 기본으로 둔다.
- `Vib_*`는 오프셋이 포함된 센서 raw 레벨일 가능성이 높아, **절대 단위 환산은 보류**하고 상대 비교 지표로 사용한다.
...
| RPM | `FB_Rpm` | 분당 회전수 | rpm | 값 ÷ 10 | 저속 구간 중심 |
| 온도 | `Temp00~Temp19` | 온도 센서 20채널 | °C | 값 ÷ 10 | 대표값 생성 권장(평균) |
| 압력 | `Press00~Press15` | 압력 센서 16채널 | kPa | 값 ÷ 10 | 대표값 생성 권장(평균) |
